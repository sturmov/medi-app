// ============================================================================
// psy-date - cienki wrapper na <input type="date"> zgodny z systemem psy-*.
// API: label, value, placeholder, field-id, name, data-field, min, max, step,
// hint, disabled, required, read-only. Emituje `psy-change` z { value }.
// ============================================================================

import { LitElement, html, ifDefined } from './lit.js';
import './psy-label.js';

export class PsyDate extends LitElement {
    static properties = {
    ...
    _groupClasses() {
        const classes = ['form-group', 'psy-form-field', 'psy-form-field--date'];
        if (this.groupClass) {
...
if (!customElements.get('psy-date')) {
    customElements.define('psy-date', PsyDate);
}
